...
class Timer extends Map {
  constructor(options = {}) {
    ...
    debug(conf.get('storage:path'))
    this[storage] = levelup(conf.get('storage:path'), {
      valueEncoding: 'json'
    , db: require(conf.get('storage:backend'))
    , cacheSize: conf.get('storage:cache')
    })
...
timers.create(id, options, (err) => {
  ...
  create(id, body, cb) {
    ...
    this[storage].put(id, data, (err, res) => {
   ...
   **/
  remove(id, cb) {
    this[storage].del(id, (err, res) => {
      if (err) return store('unable to purge %s', id, err)
      store('%s purged from storage', id)
    })
    const t = this.get(id);
    if( !t ) {
