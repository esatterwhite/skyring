...
exports.shutdown = function shutdown(cb) {
  ...
  const run = (obj) => {
    ...
    nats.client.publish('skyring', JSON.stringify(data), () =>{
      ...
      if (count === size) return setImmediate(nats.quit, cb)
    });
  }
...
exports.watch = function( key, cb ){
  if(BAIL) return
  debug('nats subscribe: skyring')
  NATS_SID = nats.client.subscribe('skyring', {queue: 'skyring'}, (data) => {
    if(BAIL) return
    const value = json.parse(data)
    rebalance('rebalance %s', value.value.count )
